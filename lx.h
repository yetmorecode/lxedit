...
typedef struct lx_name lx_name;
struct lx_name {
	lx_name *prev;
	lx_name *next;
	uint8_t length;
...
struct exe {
	...
	Elf32_Ehdr *debug_header;
	Elf32_Shdr *debug_sections;
	uint8_t **debug_data;
};

int lx_fixup_parse(char *, lx_fixup *);
int lx_fixup_write(char *, lx_fixup *);
int lx_fixup_length(char *);
exe *lx_load(char *);
exe *lx_merge(exe *, exe *, bool);
void lx_save(exe *);
void lx_read(exe *, uint16_t, uint32_t, unsigned char *, int);
